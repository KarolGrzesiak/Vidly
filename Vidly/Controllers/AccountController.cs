        ...
        private void CreateAdminIfNeeded()
        {
            ...
            if (objAdminUser == null)
            {
                ...
                if (!RoleManager.RoleExists(RoleName.Admin))
                {
                    ...
                    IdentityRole objAdminRole = new IdentityRole(RoleName.Admin);
                    RoleManager.Create(objAdminRole);
                }
                // Create Admin user
                var objNewAdminUser = new ApplicationUser { UserName = AdminUserName, Email = AdminUserName, Phone = "1", DrivingLicense = "1" };
                var AdminUserCreateResult = UserManager.Create(objNewAdminUser, AdminPassword);
                // Put user in Admin role
                UserManager.AddToRole(objNewAdminUser.Id, RoleName.Admin);
                UserManager.AddClaimAsync(objNewAdminUser.Id, new Claim("role", RoleName.Admin));
            }
        }
